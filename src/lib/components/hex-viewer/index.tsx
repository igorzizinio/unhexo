...
import { Button, Dialog, Field, NumberField } from "@base-ui/react";
...
export function BufferedHexViewer({
  ...
  const handleGoToSubmit = (event: SubmitEvent) => {
    event.preventDefault();

    const form = event.currentTarget as HTMLFormElement;
    const data = new FormData(form);
    let input = data.get("gotoInput")?.toString().trim() ?? "";
    ...
    if (input.startsWith("0x") || input.startsWith("0X")) {
      ...
      // Se tiver letras A-F, é hex
      base = 16;
    }

    const position = parseInt(input, base);

    if (!Number.isNaN(position)) {
      setSelectedByte(position);
      setSelectionStart(position);
      setSelectionEnd(position);
    }

    setGoToOffsetOpen(false);
  };
  ...
  return (
    ...
    >
      <Dialog.Root open={goToOffsetOpen} onOpenChange={setGoToOffsetOpen}>
            ...
            >
              ...
              >
                <Field.Root className="flex flex-col gap-2" name="gotoInput">
                  <label htmlFor={"gotoInput"}>Position:</label>
                  <Field.Control
                    id={"gotoInput"}
                    className="h-9
                    border border-border
                ...
                <div className="mt-6 flex justify-end gap-2">
                  <Dialog.Close
                    className={`inline-flex h-9 items-center justify-center rounded-md px-4 text-sm font-medium transition-colors border border-border bg-background hover:bg-accent hover:text-accent-foreground`}
                  >
                    Cancel
                  </Dialog.Close>
