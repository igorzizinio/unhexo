...
export function BufferedHexViewer({
	...
	const getByteValue = useCallback(
		...
		[visibleBuffer, visibleStartOffset],
	);
	...
	const handleKeyDown = useCallback(
		async (e: KeyboardEvent) => {
			...
			if (/^[0-9A-F]$/.test(key)) {
				...
				// For now, we need to load the full buffer to modify
				// In a production app, you'd want to track modifications separately
				const fullBuffer = await fileBuffer.readBytes(0, fileSize);
				fullBuffer[selectedByte] = newByte;
				updateTabBuffer(tabId, fullBuffer);
				onHasChanged?.(true);
				return;
			}
					...
					>
						...
						{maxVirtualScroll > 0 && (
							...
							>
								...
								<div
									className="scrollbar-thumb absolute right-0 w-3 bg-primary/60 hover:bg-primary/80 rounded cursor-grab active:cursor-grabbing transition-colors"
									style={{
